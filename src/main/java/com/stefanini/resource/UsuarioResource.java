...
public class UsuarioResource {
    ...
    public Response listar(){
        return Response.ok(usuarioService.listarUsuarios()).status(Response.Status.OK).build();
    }

    @POST
    @Consumes(MediaType.APPLICATION_JSON)
    public Response salvar ( @Valid Usuario usuario) throws Exception {
        return Response.ok(usuarioService.salvarUsuario(usuario)).status(Response.Status.CREATED).build();
    }
    ...
    public Response atualizar (@PathParam("id") @Valid Long id, Usuario usuario){
        return Response.ok(usuarioService.atualizarUsuario(id, usuario)).build();
    }
}
